...
public class BusinessLogicTests : ApiTestBase
{
    ...
    public async Task CreateCustomerAndRequest()
    {
        ...
        var customerCreateResult = await result.Content.ReadFromJsonAsync<ResultModel<Customer>>();
        Assert.NotNull(customerCreateResult);
        Assert.Equal("Create", customerCreateResult.Action);
        var customer = customerCreateResult.Result;
        ...
        var recordCreateResult = await result2.Content.ReadFromJsonAsync<ResultModel<GarageAPI.DataBase.Tables.Record>>();
        Assert.NotNull(recordCreateResult);
        Assert.Equal("Create", recordCreateResult.Action);
        var record = recordCreateResult.Result;
    ...
    public async Task GetCustomerAndCreateRequest()
    {
        ...
        var customerCreateResult = await result.Content.ReadFromJsonAsync<ResultModel<Customer>>();
        Assert.NotNull(customerCreateResult);
        Assert.Equal("Get", customerCreateResult.Action);
        var customer = customerCreateResult.Result;
        Assert.Equal(request.Email, customer.Email);
        Assert.Equal(request.FirstName, customer.FirstName);
        Assert.Equal(request.LastName, customer.LastName);
